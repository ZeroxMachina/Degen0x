...
function CourseCard({ course, completedXP }: { course: Course; completedXP: number }) {
  ...
  return (
    <Link href={`/courses/${course.slug}`} className="group block" aria-label={`${course.title} — ${LEVEL_META[course.level].label} level, ${course.totalXP} XP`}>
      <div className="bg-[#1c1c1e] rounded-2xl overflow-hidden border border-[#2c2c2e] hover:border-[#48484a] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5">
        ...
        {isStarted && !isCompleted && (
          <div className="h-0.5 bg-[#2c2c2e]" role="progressbar" aria-valuenow={completedPct} aria-valuemin={0} aria-valuemax={100} aria-label={`${course.title} progress`}>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-sm text-[#636366]">
          ...
          {completedCourses.length > 0 && (
            <div className="flex flex-wrap gap-3 mb-6" role="status" aria-live="polite">
              <div className="bg-[#1c1c1e] rounded-xl px-4 py-2 border border-[#2c2c2e] min-w-[120px]">
              ...
              <div className="bg-[#1c1c1e] rounded-xl px-4 py-2 border border-[#2c2c2e] min-w-[100px]">
          ...
          {/* Filter Pills */}
          <div className="flex gap-2 flex-wrap" role="tablist" aria-label="Filter courses by level or theme">
            {filterButtons.map((f) => (
              <button
                key={f.key}
                ...
                aria-label={`Filter courses by ${f.label}`}
                className={`flex items-center gap-1.5 px-4 py-2.5 min-h-[44px] rounded-full text-sm font-medium border transition-all ${
                  filter === f.key
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mt-8 grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.title} className="bg-[#0d0d0d] rounded-2xl p-5 border border-[#1c1c1e] opacity-70" aria-disabled="true">
