...
export default function LoanCalculatorPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <h1 className="text-4xl font-bold text-[var(--color-text)] mb-4">Crypto Loan Calculator</h1>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          <div><label className="block text-sm font-medium text-[var(--color-text)] mb-2">Annual Interest Rate (%)</label><input type="number" step="0.1" value={apr} onChange={(e) => setApr(Number(e.target.value))} className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-4 py-3 text-[var(--color-text)]" /></div>
          <div><label className="block text-sm font-medium text-[var(--color-text)] mb-2">Loan Duration (months)</label><input type="number" value={months} onChange={(e) => setMonths(Number(e.target.value))} className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-4 py-3 text-[var(--color-text)]" /></div>
        </div>
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-6">
          <h2 className="text-lg font-semibold text-[var(--color-text)] mb-6">Loan Cost Breakdown</h2>
          <div className="space-y-4">
            <div className="flex justify-between pb-4 border-b border-[var(--color-border)]"><span className="text-[var(--color-text-secondary)]">Loan Amount</span><span className="text-[var(--color-text)] font-semibold">${loanAmount.toLocaleString()}</span></div>
            ...
            <div className="flex justify-between pt-2"><span className="text-[var(--color-text)] font-semibold">Total Repayment</span><span className="text-[var(--color-text)] font-bold text-xl">${totalCost.toFixed(2)}</span></div>
          </div>
        </div>
      </div>
      <div className="mt-12 prose prose-invert max-w-none">
        <h2 className="text-2xl font-bold text-[var(--color-text)] mb-4">About This Calculator</h2>
