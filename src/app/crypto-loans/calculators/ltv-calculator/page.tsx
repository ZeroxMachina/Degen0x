    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div><label className="block text-sm font-medium text-white mb-2">Collateral Value ($)</label><input type="number" value={collateralValue} onChange={(e) => setCollateralValue(Number(e.target.value))} className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-4 py-3 text-white" /></div>
          ...
          <div><label className="block text-sm font-medium text-white mb-2">Liquidation Threshold (%)</label><input type="number" value={liquidationThreshold} onChange={(e) => setLiquidationThreshold(Number(e.target.value))} className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-4 py-3 text-white" /></div>
        </div>
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-6">
          <h2 className="text-lg font-semibold text-white mb-6">LTV Analysis</h2>
          <div className="space-y-4">
            ...
            <div className="flex justify-between pb-4 border-b border-[var(--color-border)]"><span className="text-[var(--color-text-secondary)]">Price Drop to Liquidation</span><span className="text-white font-semibold">{priceDropToLiquidation.toFixed(1)}%</span></div>
            <div className="flex justify-between pt-2"><span className="text-white font-semibold">Safe Max Borrow</span><span className="text-white font-bold text-xl">${maxBorrow.toFixed(0)}</span></div>
          </div>
        </div>
      </div>
    </div>
