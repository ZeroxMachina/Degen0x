...
function PerformanceChart({ data }: { data: PortfolioDataPoint[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ background: S.surface2, borderRadius: "8px", width: "100%", height: "auto" }} preserveAspectRatio="xMidYMid meet" aria-label="Portfolio 30-day performance chart">
      <defs>
...
/* ─── Main Component ──────────────────────────────────────────────── */
export default function PortfolioTrackerClient() {
  ...
  const handleAddWallet = () => {
    ...
    if (!isValidEthereumAddress(newWalletAddress)) {
      setAddressError("Invalid Ethereum address. Must start with 0x and contain 40 hex characters.");
      return;
    }
    setAddressError("");
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 20px", display: "grid", gridTemplateColumns: "1fr", gap: "24px" }}>
        ...
        <div style={{ background: S.surface, borderRadius: "12px", padding: "20px", border: `1px solid ${S.border}` }}>
            ...
            <div style={{ background: S.surface2, padding: "16px", borderRadius: "8px", marginBottom: "16px", border: `1px solid ${S.border}` }}>
              <input
                ...
                style={{
                  background: S.bg,
                  color: S.text,
                  border: `1px solid ${addressError ? "#f85149" : S.border}`,
                  padding: "8px 12px",
                  borderRadius: "6px",
                  width: "100%",
                  marginBottom: addressError ? "4px" : "10px",
                  fontSize: "13px",
                }}
              />
              {addressError && (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {wallets.length === 0 ? (
              <div style={{ color: S.text2, textAlign: "center", padding: "20px", fontSize: "13px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", marginLeft: "auto" }}>
            ...
            {(["value", "change"] as const).map((sort) => (
              <button
                ...
                aria-label={`Sort by ${sort === "value" ? "portfolio value" : "24 hour change"}`}
                style={{
