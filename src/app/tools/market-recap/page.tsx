...
function ImpactBadge({ impact }: { impact: MarketEvent["impact"] }) {
  ...
  return (
    <span className={`text-xs font-bold px-2 py-0.5 rounded-md border ${styles[impact]} uppercase`}>
      {impact}
    </span>
  );
}
...
function CategoryBadge({ category }: { category: MarketEvent["category"] }) {
  ...
  return (
    <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${styles[category]}`}>
      {category}
    </span>
  );
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <nav className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)] mb-8">
        ...
        <span>/</span>
        <span className="text-[var(--color-text)]">Market Recap</span>
      </nav>
            ...
            {TOP_GAINERS.map((t) => (
              <div key={t.symbol} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="text-sm font-bold text-green-400">+{t.change}%</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {TOP_LOSERS.map((t) => (
              <div key={t.symbol} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="text-sm font-bold text-red-400">{t.change}%</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div key={i} className="flex gap-4 relative">
              ...
              <div className="pb-6 flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-xs font-mono text-[var(--color-text-secondary)]">{event.time}</span>
